package io.github.huahill.vadmin;

import static org.assertj.core.api.Assertions.assertThat;

import com.vaadin.flow.component.dependency.StyleSheet;
import com.vaadin.flow.theme.Theme;
import com.vaadin.flow.theme.aura.Aura;
import java.util.Arrays;
import org.junit.jupiter.api.Test;

class DefaultApplicationShellTest {
    @Test
    void loadsAuraAsTheDefaultVaadinAppearance() {
        var stylesheets = Arrays.stream(DefaultApplicationShell.class.getAnnotationsByType(StyleSheet.class))
                .map(StyleSheet::value)
                .toList();

        assertThat(DefaultApplicationShell.class.getAnnotation(Theme.class)).isNull();
        assertThat(stylesheets).containsExactly(Aura.STYLESHEET);
    }
}
